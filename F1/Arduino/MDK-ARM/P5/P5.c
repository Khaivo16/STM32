...
unsigned int const timer_clock[8]={18,36,72,144,288,575}; // he so chia nap vao timer
unsigned char vitri_bit=0,row=0; // kiem tra xem dang quet den bit nao
unsigned char hang_led;
uint16_t led[MAX_BIT][64];
unsigned char display[3][32][64];
...
{  
	for(int ts=0;ts<MAX_BIT;ts++)
	{
	 ...
	 {
		 led[ts][i]=0;
		 if((display[1][16+hang][i] & (1<<ts)) != 0 ){led[ts][i]|=(0x001<<G2);} //ghi bit4  G2 
		 if((display[1][0+hang][i] & (1<<ts)) != 0 ){led[ts][i]|=(0x001<<G1);}   //ghi bit2  G1
		 if((display[0][0+hang][i] & (1<<ts)) != 0 ){led[ts][i]|=(0x001<<R1);}   //ghi bit1  R1
		 if((display[2][0+hang][i] & (1<<ts)) != 0 ){led[ts][i]|=(0x001)<<B1;}   //ghi bit3  B1
		 if((display[0][16+hang][i] & (1<<ts)) != 0 ){led[ts][i]|=(0x001)<<R2;} //ghi bit5  R2
		 if((display[2][16+hang][i] & (1<<ts)) != 0 ){led[ts][i]|=(0x001)<<B2;} //ghi bit6  B2
	 }
	} 
}
uint8_t toggle=0;
void ngatquetled(void)
{
  ...
  TIM2->CCR4 = 0;//__HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_4,0);
	...
	for(int i=0;i<64;i++){data_PORT->ODR=led[vitri_bit][i];;clk_P->BSRR=(1<<(clk+16));clk_P->BSRR=(1<<clk);}	 								
	hang(hang_led);
...
void MATRIX_putStr16x16(signed int x,signed int y,unsigned char a[],unsigned char r,unsigned char g,unsigned char b){
	unsigned char i=0;
	while(a[i]!=0x00){
		//P5_chonvitri(x,y);P5_sendString_wColor(S{a[i]},r,g,b);
