...
uint8_t MAX30100::burstRead(uint8_t baseAddress, uint8_t *buffer, uint8_t length)
{
    Wire.beginTransmission(MAX30100_I2C_ADDRESS);
...
void MAX30100::readFifoData()
{
    ...
    if (toRead) {
        if(burstRead(MAX30100_REG_FIFO_DATA, buffer, 4 * toRead)==0) return;
				
				SensorReadout a;
        for (uint8_t i=0 ; i < toRead ; ++i) {
            // Warning: the values are always left-aligned
					a.ir=(uint16_t)((buffer[i*4] << 8) | buffer[i*4 + 1]);
					a.red=(uint16_t)((buffer[i*4 + 2] << 8) | buffer[i*4 + 3]);				
          readoutsBuffer.push(a);
        }
    }
}
