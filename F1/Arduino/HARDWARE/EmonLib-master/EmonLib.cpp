...
double getICAL(double Irms)
{
    if (Irms < 1.0) return 4.0;
    else return 4.5;
}
//------------------x--------------------------------------------------------------------
...
//  #endif
  ...
  {
    sampleI = adcRead(inPinI,ADC_12_BIT);
//		float sampleI_filtered = filter_ADC(sampleI);
    // Digital low pass filter extracts the 2.5 V or 1.65 V dc offset,
    //  then subtract this - signal is now centered on 0 counts.
    offsetI = (offsetI + (sampleI-offsetI)/2048.0);
    filteredI = sampleI - offsetI;
