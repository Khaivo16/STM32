...
void MFRC522(int sad, int reset);
  ...
  void reset(void);
  byte RC522_getFirmwareVersion(void);
  boolean digitalSelfTestPass(void);
  int commandTag(byte command, byte *data, int dlen, byte *result, int *rlen);
  int RC522_requestTag(byte mode, byte *type);
  int RC522_antiCollision(byte *serial);
  void calculateCRC(byte *data, int len, byte *result);
  byte RC522_selectTag(byte *serial);
  int RC522_authenticate(byte mode, byte block, byte *key, byte *serial);
  int RC522_readFromTag(byte blockAddr, byte *recvData);
