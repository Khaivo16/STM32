...
size_t TwoWire::write(uint8_t data)
{
  ...
  if (transmitting) {
    ...
    // error if no memory block available to allocate the buffer
    if (allocateTxBuffer(txBufferLength + 1)  == NULL) {
      //setWriteError();
      ret = 0;
    } else {
...
size_t TwoWire::write(const uint8_t *data, size_t quantity)
{
  ...
  if (transmitting) {
    ...
    // error if no memory block available to allocate the buffer
    if (allocateTxBuffer(txBufferLength + quantity) == NULL) {
      //setWriteError();
      ret = 0;
    } else {
...
{
  pinMode(_i2c.sda, INPUT);

  if (digitalRead(_i2c.sda) == LOW) {
    pinMode(_i2c.scl, OUTPUT);

    for (int i = 0; i < 20; i++) {
      digitalWrite(_i2c.scl, LOW);
      delayMicroseconds(10);
      digitalWrite(_i2c.scl, HIGH);
      delayMicroseconds(10);
    }
    pinMode(_i2c.scl, INPUT);
  }
}
