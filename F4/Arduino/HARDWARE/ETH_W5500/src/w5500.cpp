...
void W5500Class::init(SPIClass & _spi, uint8_t _ss_pin)
{
  initSS(_ss_pin);
  //SPI = _spi;
  //SPI.beginTransaction(SPISettings(42000000));
	SPI.begin();
...
void W5500Class::write(uint16_t _addr, uint8_t _cb, uint8_t _data)
{
    select_SS();  
    SPI.write16(_addr);
    SPI.write16((uint16_t)(_cb<<8) | _data);
    deselect_SS();
}
...
uint16_t W5500Class::read16(uint16_t _addr, uint8_t _cb)
{
    ...
    SPI.write16(_addr);
    SPI.write(_cb);
	//uint16_t _data = SPI.transfer16(0);
	uint16_t _data = (SPI.transfer(0)<<8)|SPI.transfer(0);
    deselect_SS();
    return _data;
}
