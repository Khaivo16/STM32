...
CheckBoxPaint(tWidget *pWidget, unsigned long bClick)
{
    ...
    if(!bClick)
    {
        ...
        GrRectFill(&sCtx, &sRect);
            sRect.sXMin = sRect.sXMin  + 2;
        sRect.sYMin = sRect.sYMin  + 2;
        sRect.sXMax =  sRect.sXMax - 2;
        sRect.sYMax = sRect.sYMax- 2;
            GrContextForegroundSet(&sCtx,pCheck->ulFillColor);
        GrRectFill(&sCtx, &sRect);
    }
    ...
    else
    {
     ...
     GrCircleFill(&sCtx, (sRect.sXMin+sRect.sXMax)/2, (sRect.sYMin+sRect.sYMax)/2, (sRect.sXMax-sRect.sXMin-8)/2);
